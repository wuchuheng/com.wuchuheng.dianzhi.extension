...
describe('conversation view reducer', () => {
  ...
  it('tracks mode, expansion, new selections, and validated panel handoff', () => {
    ...
    state = reduceConversationView(state, { type: 'conversation.sync', snapshot })
    expect(state).toMatchObject({ visible: false, panelOpen: true, snapshot })
    state = reduceConversationView(state, { type: 'panel.closed', conversationId: 1 })
    expect(state).toMatchObject({ visible: false, panelOpen: false })
  })
})
